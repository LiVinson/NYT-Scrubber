...
class Home extends Component {
    ...
    API.getArticles(this.state)
      .then(res => {
        ...
        this.setState({
          ...
          error: ""
        }); //Saves first 5 search results into state as an array of objects
      })
  ...
  handleArticleSave = event => {
    event.preventDefault();
    const clickedArticle = this.state.results.filter(
  ...
  handleArticleDelete = event => {
    event.preventDefault();
    const clickedArticle = this.state.savedArticles.filter(
      element => (element._id = event.target.id)
    )[0]; //Locates the article from the savedArticle array with the ID matching the button clicked
    ...
    return (
      <Container>
        <Jumbotron>
          ...
          <h3>
            Search for NYT articles using a keyword and save any article to
            review later
          </h3>
        </Jumbotron>
          ...
          {this.state.results.length ? (
            <List>
              {this.state.results.map((
                result //For each article in the result array, create a list item with a div w/ article info, and a save button with article ID
              ) => (
          ...
          {this.state.savedArticles.length ? (
            <List>
              {this.state.savedArticles
                ...
                .reverse()
                .map((
                  article //For each article in the savedArticles array, create a list item (reverse order) with a div w/ article info, and a delete button with article ID
                ) => (
        ...
        <Footer>
          {" "}
          <a
            href="https://livinson.github.io/LisaVinson/"
            target="_blank"
